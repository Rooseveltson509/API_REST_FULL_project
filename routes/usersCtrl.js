...
module.exports = {
  register: function (req, res) {
    ...
    if (nom.length >= 50 || nom.length <= 1) {
      return res
        .status(400)
        .json({ error: "wrong first name (must be length 2 - 50)" });
    }
    ...
    if (prenom.length >= 50 || prenom.length <= 1) {
      return res
        .status(400)
        .json({ error: "wrong last name (must be length 2 - 50)" });
    }
      ...
      [
        function (done) {
          models.User.findOne({
            ...
            .catch(function (err) {
              return res
                .status(500)
                .json({ error: "Important de verifier cet utilisateur" });
            });
        },
        ...
        function (userFound, bcryptedPassword, done) {
          let newUser = models.User.create({
            ...
            .catch(function (err) {
              return res.status(500).json({ error: "Important d'ajouter cet utilisateur" });
            });
        },
      ],
      function (newUser) {
        ...
        function (userFound, done) {
          if (userFound) {
            userFound
              ...
              .catch(function (err) {
                res.status(500).json({ error: "Impossible de vérifier cet user" });
              });
          } else {
